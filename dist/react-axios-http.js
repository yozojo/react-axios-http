(function (global, factory) {
	...
	Interceptor.prototype.forEach = function forEach(fn) {
	  lodash.forEach(this.handlers, function (h) {
	    if (h !== null) {
	      fn(h);
	    }
	  });
	...
	var defineProperty$2 = function defineProperty(target, props) {
	  ...
	  lodash.forEach(props, function (prop) {
	...
	var getScope = function getScope(arr, IO, isScope) {
	  ...
	  try {
	    var isDeep = isType(lodash.values(IO)[0], 'object');
	    ...
	    return lodash.reduce(arr, function (pre, _ref2) {
	      var _extends2;
	...
	var getOption = function getOption(scope, IO) {
	  ...
	  if (isType(scope, 'object')) {
	    option = lodash.assign(option, scope);
	    scope = option.scope;
	  }

	  scope = isType(scope, 'string') ? [scope] : scope;

	  var scopes = lodash.filter(apis, function (_ref3) {
	    var key = _ref3[0];
